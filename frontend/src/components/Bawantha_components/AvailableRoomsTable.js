      ...
      <table className="w-full border-collapse border border-gray-300">
        <thead>
          <tr className="bg-gray-200 text-center">
            ...
            <th className="border p-2 w-1/6">Checkout</th>
          </tr>
        </thead>
        <tbody>
          {sortedRooms.length > 0 ? (
            sortedRooms.map((room) => (
              <tr key={room.roomNumber} className="text-center">
                ...
                <td className="border p-2">
                  <button
                    className="bg-blue-500 text-white shadow-md rounded-md px-3 py-1 hover:bg-blue-600 transition"
                    onClick={() => navigate(`/checkout/${room.roomNumber}`)}
                  >
                    Checkout
                  </button>
                </td>
